...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        $customersdata = Customer::orderBy('id', 'desc') // Sort by the latest customers
            ...
            ->get();
        $topProducts = Product::select('products.id', 'products.name', 'products.status', DB::raw('COUNT(product_orders.id) as order_count'))
        ...
        foreach ($salesData as $year => $sales) {
            if ($previousSales !== null) {
                $growthRates[$year] = $this->calculateGrowth($previousSales, $sales);
            } else {
                $growthRates[$year] = 0;
            }
            $previousSales = $sales;
        }
        $latestOrders = Order::select('orders.*', DB::raw('CONCAT(customers.first_name, " ", customers.last_name) as customer_name'))
            ->leftJoin('customers', 'orders.customer_id', '=', 'customers.id')
            ->orderBy('id', 'desc')
            ->limit(5)
            ->get();

        return view('pages.dashboard', compact('category', 'orders', 'orders_products', 'growthRates', 'salesData', 'customers', 'totalorders', 'customersdata', 'topProducts', 'latestOrders'));
    }
