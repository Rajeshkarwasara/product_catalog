...
class SubCategoryController extends Controller
{
    public function __construct(Request $request)
    {
        ...
        $this->sortableColumns = ['id', 'name', 'category_id', 'status', 'created_at'];
    }

    public function index()
    {
        $Category = Category::whereNull('deleted_at')->orderBy('name', 'asc')->get();
        $Ucategory = Category::whereNull('deleted_at')->has('subcategories')->orderBy('name', 'asc')->get();

        return view('content/SubCategory/list', compact('Category', 'Ucategory'));
    }
    ...
    public function list(Request $request)
    {
        ...
        $search_data['select_category'] = $request->input('select_category');
        ...
        foreach ($data as $value) {
            ...
            $row['image'] = '<div class="table_img_"><img src="/uploads/SubCategory/' . $value->image . '" alt="Image"></div>';
    ...
    {
        $q = SubCategory::select('subcategory.*', 'category.name as category_name')
            ->leftjoin('category', 'category.id', '=', 'subcategory.category_id')
            ->where('subcategory.id', '!=', '0');
        // print_r($q);die;
        ...
        if ($search && !empty($search)) {
            $q->where(function ($query) use ($search) {
                $query->where('subcategory.name', 'LIKE', '%' . $search . '%');
            });
        }
        if (isset($request['status']) && !empty($request['status'])) {
            $q->where(function ($query) use ($request) {
                $query->where('subcategory.status', $request['status']);
            });
        }
        if (isset($request['name']) && !empty($request['name'])) {
            $q->where(function ($query) use ($request) {
                $query->where('subcategory.name', 'LIKE', '%' . $request['name'] . '%');
            });
        }
        if (isset($request['select_category']) && !empty($request['select_category'])) {
            $q->where(function ($query) use ($request) {
                $query->where('subcategory.category_id', $request['select_category']);
            });
        }
    ...
    public function store(Request $request)
    {
        ...
        $msg = ['name.unique' => 'The subcategory has already been taken.'];

        if (isset($request->id) && $request->id > 0) {
            $rules['name'] = 'required|unique:subcategory,name,' . $request->id . ',id,deleted_at,NULL';
            $validator = Validator::make($request->all(), $rules, $msg);
        } else {
            $rules['name'] = 'required|unique:subcategory,name,NULL,id,deleted_at,NULL';
            $validator = Validator::make($request->all(), $rules, $msg);
        }
        ...
        try {
            ...
            if (isset($request->id) && $request->id > 0) {
                ...
                if ($profile_image = $request->file('image')) {
                    $destination_path = 'uploads/SubCategory';
                ...
                $item->save();
                if ($item) {
                    $res = array('code' => 200, 'msg' => 'Updated successfully', 'category_id' => $request->category_id);
                } else {
                    $res = array('code' => 201, 'msg' => 'Failed! Try again', 'category_id' => $request->category_id);
                }
            } else {
                //store
                ...
                $item->save();
                if ($item) {
                    $res = array('code' => 200, 'msg' => 'Added successfully', 'category_id' => $request->category_id);
                } else {
                    $res = array('code' => 201, 'msg' => 'Failed! Try again', 'category_id' => $request->category_id);
                }
            }
    ...
    {
        try {
            $item = SubCategory::select('subcategory.*', 'category.name as category_name')
                ->leftjoin('category', 'category.id', '=', 'subcategory.category_id')
                ->where('subcategory.id', $request->id)->first();
            //   echo "<pre>";
            //   print_r($item) ; die ;
    ...
    public function delete(Request $request)
    {
        ...
        try {
            ...
            if ($status === true) {
                return response()->json(['success' => 'Deleted successfully', "status" => $status, 'category_id' => $id], 200);
            } else {
                return response()->json(['error' => 'Something went wrong', "status" => $status, 'category_id' => $id], 201);
            }
        } catch (Throwable $e) {
            report($e);
