...
class SliderController extends Controller
{
    ...
    protected $page = 'content/slider';
    //this variable use for route prefix and view folder 
    protected $routeLable = 'slider';
    ...
    public function getData($search = null, $orderby = null, $order = null, $request = null)
    {
        $q =    Slider::select('*');
        //  print_r($q);die;
        $orderby = $orderby ? $orderby : 'sliders.id';
        ...
        if (isset($request['title']) && $request['title'] != '') {
            $q->where('sliders.title', 'LIKE', '%' . $request['title'] . '%');
        }
        if (isset($request['customer_type']) && $request['customer_type'] != '') {
            $q->whereIn('sliders.customer_type', [$request['customer_type'], 'all']);
        }
        if (isset($request['status']) && $request['status'] != '') {
            $q->where('sliders.published_status', $request['status']);
        }
 ...
 public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $response = $response->offset($start)->limit($limit)->orderBy('sliders.id', 'desc')->get();
            if (!$response) {
                $data = [];
                $paging = [];
            } else {
            ...
            foreach ($data as $value) {
                ...
                $row['image'] = '<div class="table_img_"><img src="/uploads/slider_image/' . $value->image . '" alt="Image"></div>';
...
// exit;
        ...
        $publishstatus = Slider::groupBy('published_status')->get();
    ...
    public function store(Request $request)
    {
        ...
        if (isset($request->id) && $request->id > 0) {
            ...
            if ($image = $request->file('image')) {
                $destination_path = 'uploads/slider_image';
                $source_path = $image;
    ...
    public function get_by_id(Request $request)
    {
        ...
        try {
            ...
            if (!$slider) {
                return response()->json(['code' => 404, 'msg' => 'Slider not found'], 404);
            }
            $data = array();
            $data = ['slider' => $slider];
    ...
    public function view($id, Request $request)
    {
        try {
            ...
            $data = array();
            $data['slider'] = $blog_detail =  Slider::select('sliders.*')
                ->findOrFail($id);
    ...
    public function delete(Request $request)
    {
        ...
        try {
            ...
            if ($status === true) {
                return response()->json(['success' => 'Slider deleted successfully', "status" => $status], 200);
            } else {
