...
class CategoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('content/Category/list', compact('Category'));
    }
    ...
    public function list(Request $request)
    {
        ...
        foreach ($data as $value) {
            ...
            $row['status'] = $val;
            $row['image'] = '<div class="table_img_"><img src="/uploads/Category/' . $value->image . '" alt="Image"></div>';
            ...
            // $status .= '<a href="' . url('getsubcategory/' . $value->id) . '" title="Subcategory"><i class="bx bx-folder-open f-16 text-blue"></i></a>';
    ...
    public function getData($search = null, $orderby = null, $order = null, $request = null)
    {
        ...
        if (isset($request['name']) && !empty($request['name'])) {
            $q->where(function ($query) use ($request) {
                $query->where('category.name', 'LIKE', '%' . $request['name'] . '%');
            });
        }
    ...
    public function store(Request $request)
    {
        ...
        $msg = ['name.unique' => 'The category has already been taken.'];

        if (isset($request->id) && $request->id > 0) {
            $rules['name'] = 'required|unique:category,name,' . $request->id . ',id,deleted_at,NULL';
            $validator = Validator::make($request->all(), $rules, $msg);
        } else {
            $rules['name'] = 'required|unique:category,name,NULL,id,deleted_at,NULL';
            $validator = Validator::make($request->all(), $rules, $msg);
        }
    ...
    {
        $category_id = $id;
        $subcategory = SubCategory::select('subcategory.*', 'category.name as category_name')
            ->leftjoin('category', 'category.id', '=', 'subcategory.category_id')
            ->where('subcategory.category_id', $category_id)->get();
        $Category = Category::whereNull('deleted_at')->has('subcategories')->get();


        return view('content/Category/subcategory', compact('subcategory', 'Category', 'category_id'));
    }
};
